...
# RAGFlow Deeper Search Script with DSPy Query Deepening AND Reranking
# Usage: deeper_search_reranked_ragflow.sh -k KNOWLEDGE_BASE -n NUMBER_OF_HITS -d DEEPENING_LEVEL "query"
# Example: deeper_search_reranked_ragflow.sh -k BASF -n 10 -d 2 "what is a volatility clock"
# NOTE: This script will demonstrate the known rerank protocol error
...
usage() {
    ...
    echo ""
    echo "NOTE: This script uses both DSPy deepening AND reranking to demonstrate the known protocol error."
    echo ""
    echo "Example:"
...
read_config() {
    ...
    RAGFLOW_DEFAULT_RERANK=$(jq -r '.RAGFLOW_DEFAULT_RERANK // "Cohere/rerank-multilingual-v3.0"' "$CONFIG_FILE")
...
# Function to search knowledge base with DSPy deepening AND reranking
search_knowledge_base_deepened_reranked() {
    local dataset_id="$1"
    local query="$2"
    local page_size="$3"
    ...
    echo "Sending request with rerank_id: $rerank_id"
...
format_results_deepened_reranked() {
    local response="$1"
    ...
    if [[ "$code" != "0" ]]; then
        ...
        message=$(echo "$response" | jq -r '.message // "Unknown error"')
        echo "========================================================"
        echo "EXPECTED ERROR - Known Rerank Protocol Issue:"
        echo "========================================================"
        echo "Error Code: $code"
        echo "Error Message: $message"
        echo ""
        echo "This error demonstrates the known rerank protocol issue documented in the README."
        echo "The rerank functionality currently has a protocol error that prevents it from working."
        echo "See README.md 'Known Issues' section for details."
        echo ""
        echo "To use DSPy deepening without reranking, use: deeper_search_ragflow.sh"
        return 1
    fi
    ...
    echo "Found $total_results total results with DSPy query deepening AND reranking:"
    echo "========================================================================"
    echo ""
    ...
    echo "Reranking: ENABLED (if this appears, the protocol error was fixed)"
    echo ""
...
echo "Using reranking with model: $RAGFLOW_DEFAULT_RERANK"
echo "Requesting top $NUM_HITS results..."
echo ""
echo "WARNING: This script will likely show the known rerank protocol error."
echo ""

DATASET_ID=$(get_dataset_id "$KNOWLEDGE_BASE")
RESPONSE=$(search_knowledge_base_deepened_reranked "\"$DATASET_ID\"" "$QUERY" "$NUM_HITS" "$DEEPENING_LEVEL" "$RAGFLOW_DEFAULT_RERANK")
format_results_deepened_reranked "$RESPONSE"