...
# RAGFlow Search Script
# Usage: search_ragflow.sh -k KNOWLEDGE_BASE -n NUMBER_OF_HITS "query"
# Example: search_ragflow.sh -k BASF -n 10 "Retrieve the income statement"
...
usage() {
    echo "Usage: $0 -k KNOWLEDGE_BASE [-n NUM_HITS] \"query\""
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Example:"
    echo "  $0 -k BASF -n 10 \"Retrieve the income statement\""
    exit 1
}
...
format_results() {
    ...
    echo "Found $total_results total results:"
    echo "=================================="
    echo ""
...
RESPONSE=$(search_knowledge_base "\"$DATASET_ID\"" "$QUERY" "$NUM_HITS")
